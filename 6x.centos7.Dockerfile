...
LABEL maintainer="dev@joeir.net" \
      version="1.0.0" \
      description="CentOS 7 container with custom built Node Boron LTS"
ENV INSTALL_PATH=/tmp
ENV YUM_TO_INSTALL="bash curl gcc gcc-c++ automake autoconf libtoolize make glibc-static libstdc++-static"
ENV YUM_TO_REMOVE="bash curl gcc gcc-c++ automake autoconf libtoolize make glibc-static libstdc++-static"
ENV YUM_DEFAULT_DEPENDENCIES_FOR_YARN_BUILD="python make gcc gcc-c++"
...
RUN printf "${FONT_BOLD}${FONT_GREEN} \n\
    ...
    echo "apk add --no-cache ${YUM_DEFAULT_DEPENDENCIES_FOR_YARN_BUILD};" >> ${YARN_PRE_INSTALL} && \
    chmod +x ${YARN_PRE_INSTALL} && \
    touch ${YARN_POST_INSTALL} && \
    echo "#!/bin/sh" > ${YARN_POST_INSTALL} && \
    echo "yarn cache clean;" >> ${YARN_POST_INSTALL} && \
    echo "apk del ${YUM_DEFAULT_DEPENDENCIES_FOR_YARN_BUILD};" >> ${YARN_POST_INSTALL} && \
